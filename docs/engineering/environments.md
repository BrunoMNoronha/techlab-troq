...
| Variável | Ambientes | Classificação | Obrigatoriedade | Estado | Origem |
| --- | --- | --- | --- | --- | --- |
| `NEXT_PUBLIC_SENTRY_DSN` | os três | **pública** — **não** é segredo | obrigatória quando a instrumentação Sentry estiver ativa | previsto | Endereço de ingestão do projeto Sentry **daquele ambiente**; um valor distinto por ambiente. É lida por código que executa no navegador, logo é pública pelo critério da seção 3.1 — e o provedor documenta que, sem DSN, a SDK não envia nada. Exigida por [../adr/0007-observability-sentry.md](../adr/0007-observability-sentry.md), decisões 10 e 12; RNF-018; AR-14.1 |
...
**Estado após F1-008 (2026-09-15).** O provedor está **escolhido** e o contrato está **escrito**; **nada está provisionado**. Não existe conta, organização ou projeto Sentry, nenhum DSN ou token real foi gerado, `@sentry/nextjs` não está instalado, não há arquivo de instrumentação e nenhuma telemetria é emitida. As quatro variáveis desta seção estão **`previsto`** e assim permanecem até que código versionado as leia. O entregável **E-7 continua não iniciado**. O provisionamento em `development` e `preview` é trabalho posterior e próprio, e a instrumentação do SDK é outro ([../delivery/backlog.md](../delivery/backlog.md)).
