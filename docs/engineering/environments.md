...
| Onde os valores vivem | Configuração de variáveis do projeto na Vercel, no escopo `preview` |
...
| `preview` | Configuração de variáveis do projeto na plataforma de deploy, escopo `preview` | Bruno, como responsável final pelo produto ([ai-agent-workflow.md](ai-agent-workflow.md), seção 1), ou quem ele designar |
