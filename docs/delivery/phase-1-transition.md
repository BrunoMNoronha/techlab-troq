...
**Atualização de 2026-09-15 (F1-009).** **E-7 continua `não iniciado`.** F1-009 fechou a parte de **provisionamento**: na organização Sentry `techlab-bt`, reutilizada por já existir e ser do TechLab+/TROQ, foram criados **dois projetos isolados**, `techlab-troq-development` e `techlab-troq-preview`, com identificadores de projeto e **client keys distintos** e **uma DSN por ambiente**, comprovadamente segregados — inclusive por evento sintético de prova, com dados fictícios, que apareceu apenas no projeto de destino e foi resolvido em seguida. No provedor, o *data scrubbing* server-side está habilitado com os **scrubbers padrão preservados** e os campos sensíveis do domínio acrescentados, **sem *safe field***, **sem armazenamento de IP**, **sem Session Replay** e **sem `sendDefaultPii`**. `NEXT_PUBLIC_SENTRY_DSN` foi custodiada em `.env.local` (`development`) e somente no escopo **Preview** da plataforma de deploy (`preview`). **Nenhum `SENTRY_AUTH_TOKEN` foi criado** e `SENTRY_ORG` e `SENTRY_PROJECT` **não** foram configuradas como variáveis, por serem condicionais a source maps e releases, não adotados aqui. **Nenhuma implementação passou a existir:** `@sentry/nextjs` continua ausente, não há arquivo de instrumentação e **nenhuma telemetria da aplicação é emitida** — as quatro variáveis seguem `previsto`, porque nenhum código versionado as lê. O próximo item é **F1-010**, a instrumentação. Nada de `production` foi criado. **A Fase 1 continua não concluída** e **nenhum critério do seu gate de saída foi verificado.**
