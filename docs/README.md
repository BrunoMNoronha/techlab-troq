...
Índice da documentação do projeto. Documentos marcados como **existente** estão versionados e vigentes — a maior parte vem do baseline da Fase 0; [engineering/environments.md](engineering/environments.md) foi produzido já na Fase 1, por F1-001, e [engineering/database.md](engineering/database.md) por F1-002, F1-003 e F1-004. Documentos marcados como **futuro** ainda não existem e serão criados progressivamente nas fases seguintes; os nomes são indicativos e podem mudar.
...
| [engineering/conventions.md](engineering/conventions.md) | existente | Convenções de engenharia: runtime Node.js 24.x LTS, Next.js 16.3.x Active LTS, TypeScript `strict`, App Router, pinning e lockfile, política de dependências, organização de código e fronteiras de módulo — incluindo, desde **F1-005**, a estrutura física `src/modules/<module>/index.ts`, com `index.ts` como API pública, interior privado por padrão, proibição de barrel global e de import que atravesse a fronteira (seção 2.5) —, fronteira servidor/cliente, dados protegidos, autorização server-side, validação de entrada, TypeScript, comandos padronizados de qualidade (incluindo `test:integration`, desde F1-003) e princípios de segurança |
| [engineering/environments.md](engineering/environments.md) | existente | Contrato de ambientes e segredos: os três ambientes `development`, `preview` e `production`, com propósito, origem dos dados, quem acessa e classe de credencial admissível; critério objetivo de classificação entre variável **pública** e **exclusivamente server-side**, com a proibição sem exceção de segredo em variável pública e a regra de que dado protegido não é variável de ambiente; convenção de nomes; catálogo por área — aplicação, PostgreSQL/Neon com a distinção entre endpoint pooled e direto, Cloudflare R2, Resend, Better Auth, Mercado Pago e segredo de agendamento —, cada variável com ambiente, classificação, obrigatoriedade, estado e origem normativa; e regras operacionais de custódia, adição, rotação, revogação e resposta a vazamento. Produzido por **F1-001** ([delivery/prompts/f1-001-environments-and-secrets.md](delivery/prompts/f1-001-environments-and-secrets.md)); espelhado em [`.env.example`](../.env.example). `DIRECT_URL` é `consumido` pelo Prisma CLI (F1-002) e `DATABASE_URL` pelo runtime do Prisma Client (F1-003); desde F1-004 os valores reais de `preview` vivem como secrets do GitHub Environment `preview` e, para a pooled, no escopo Preview da Vercel; desde **F1-006** a seção 5.3 registra o R2 provisionado em `development` e `preview`, com buckets isolados e privados e credencial própria por bucket, custodiadas em `.env.local` e no escopo Preview da Vercel; todas as variáveis do R2 — e todas as demais — continuam `previsto`, porque nenhum código versionado as lê, e o Resend continua não provisionado |
| [engineering/database.md](engineering/database.md) | existente | Materialização física do banco: Prisma ORM 7.10.0 pinado, `prisma.config.ts` com `DIRECT_URL` para o CLI e `DATABASE_URL` reservada ao runtime, estrutura dos arquivos Prisma, UUID nativo sem extensão, `timestamptz(6)`, dinheiro em centavos inteiros, convenções de nomes e ações referenciais, como a migration inicial materializa I-1, I-2, I-5, I-6 e I-8 e as demais restrições do modelo (índices únicos parciais, `CHECK`s e triggers por SQL customizado, sem Preview feature), fato verificado sobre drift, proibição de `db push` fora de desenvolvimento, comandos permitidos, o que ainda não existe (Neon, job de `migrate deploy`), o procedimento de validação sobre banco descartável e, desde **F1-003**, o runtime do Prisma Client: fronteira server-side `src/persistence/prisma.ts` com `@prisma/adapter-pg` sobre `DATABASE_URL`, inicialização lazy, instância única e prova de integração por `npm run test:integration`; e, desde **F1-004**, o Neon de `preview` (São Paulo, PostgreSQL 17, branch `preview`, database `troq`), a custódia das conexões direta e pooled, o workflow `.github/workflows/migrate-preview.yml` de `migrate deploy` somente a partir de `main`, serializado, a limitação deliberada de banco compartilhado alinhado a `main`, a imutabilidade da migration inicial desde a primeira aplicação e a evidência dessa execução. Produzido por **F1-002** e atualizado por **F1-003** e **F1-004**; aponta para [architecture/data-model.md](architecture/data-model.md) em vez de duplicá-lo |
| [engineering/testing.md](engineering/testing.md) | existente | Estratégia de testes: níveis (unitário, integração, componentes, E2E, contrato), Vitest 5.x e React Testing Library, Playwright previsto para E2E futuro, prioridade por risco, invariantes a provar para RB-001 a RB-006, requisitos futuros de concorrência e idempotência, política de test doubles e, desde F1-004, a distinção entre o CI de PR (sem banco) e a validação pós-merge contra o Neon de `preview`, com `test:integration` somente leitura |
...
| [delivery/backlog.md](delivery/backlog.md) | existente | Backlog de alto nível por fase. Todos os itens `F0-xxx` estão concluídos e a Fase 0 foi encerrada em 2026-09-14; a numeração `F1-xxx` foi aberta por F1-001, o primeiro trabalho da Fase 1; F1-001 a F1-004 estão concluídos e E-5 está concluído |
