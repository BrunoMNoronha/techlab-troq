...
Com OD-08 fechada, o item `próximo` do backlog passa a ser **F0-019**, que deve fechar OD-07 — chargebacks, duplicidade, pagamento após a expiração da reserva e falhas de confirmação — antes que F0-022 produza o design de pagamentos. **OD-07 permanece aberta e é a última decisão aberta da Fase 0.** A Fase 0 **não** está concluída. Nenhuma integração de pagamento foi implementada: não há SDK, route handler de webhook, schema, migration nem secret no repositório.
