...
Estado da Fase 0 após estas decisões: F0-017, F0-018, F0-020, F0-021 e F0-024 estão concluídos. **Não há item `próximo` nem `pendente` no backlog:** todo o trabalho restante da Fase 0 está bloqueado pela mesma raiz — a validação real do gateway de pagamento Pix para exatamente R$ 0,99 (F0-010, OD-08), cujo spike teve uma terceira execução em 2026-09-14, a primeira com Access Token de teste, classificada novamente como `INCONCLUSIVO`. Essa execução comprovou experimentalmente oito dos dez critérios — incluindo a aceitação de exatamente R$ 0,99 e a tarifa efetiva de R$ 0,01 sobre esse valor, com líquido de R$ 0,98 — e reduziu o bloqueio a dois critérios de webhook, travados por uma causa única: a configuração da URL de notificação e a chave secreta do HMAC só existem no painel “Suas integrações”, cujo acesso exige autenticação interativa. Mercado Pago continua apenas candidato (DEC-017) e ADR-0004 não pode ser criado sem evidências. A Fase 0 **não** está concluída.
