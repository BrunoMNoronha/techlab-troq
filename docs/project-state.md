...
**Não resta decisão aberta na Fase 0** e o item `próximo` do backlog passa a ser **F0-022**, a arquitetura pré-implementação. A Fase 0 **não** está concluída: o que falta é execução documental (F0-022 e F0-023), não decisão. Nenhuma integração de pagamento foi implementada: não há SDK, route handler de webhook, schema, migration nem secret no repositório.
