...
| E-4 | Configuração de ambientes e segredos (`engineering/environments.md`) | **não iniciado** | `docs/engineering/environments.md` **não existe**; [../README.md](../README.md) o indexa como `futuro`. Não existe `.env.example`. O `.gitignore` já ignora `.env` e `.env.*` com exceção para `!.env.example`, o que é pré-condição, não o entregável. **É o próximo trabalho: F1-001** |
