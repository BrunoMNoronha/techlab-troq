...
| F0-022 | Produzir arquitetura de dados e API pré-implementação | `architecture/overview.md`, `architecture/data-model.md`, `architecture/payments-design.md` e `architecture/contact-release.md`, com base nas decisões fechadas | F0-011, F0-012, F0-013, F0-014, F0-015, F0-018, F0-019, F0-020, F0-024 | próximo |
...
O que F0-019 **não** fez: nenhuma linha de integração, nenhum endpoint, nenhum receiver de webhook, nenhum schema, nenhuma migration, nenhum secret, nenhum acesso a painel produtivo e nenhum pagamento real. Com OD-07 fechada, **não resta decisão aberta na Fase 0** e **F0-022** passa a `próximo`.
