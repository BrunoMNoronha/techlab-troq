...
`npm run format` aplica a formatação e `npm run test` executa a suíte em modo de desenvolvimento. `npm run test:integration` executa a prova de integração contra um PostgreSQL descartável já migrado, apontado por `DATABASE_URL` ([docs/engineering/database.md](docs/engineering/database.md), seção 12). Os contratos normativos desses comandos estão em [docs/engineering/conventions.md](docs/engineering/conventions.md) e a estratégia de testes em [docs/engineering/testing.md](docs/engineering/testing.md).
